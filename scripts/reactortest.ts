/**
 * Phase 1 reactor acceptance runner (isolate_todo.md). Builds a real isolate from
 * a trivial program, links the reactor test driver (runtime/reactor_test_main.c),
 * and runs it plain and under MAL_GC_STRESS + MAL_GC_VERIFY.
 *
 *   node scripts/reactortest.ts [file.js]   (default tests/local/fibertest_stub.js)
 */
...
function buildBinary(file: string, name: string, compiled: boolean): string {
	...
	return buildLocalBinary({
		...
		cSource,
		verbose: false,
		mainFile: "runtime/reactor_test_main.c",
	});
}

function run(binary: string, label: string, env: NodeJS.ProcessEnv): boolean {
	process.stdout.write(`reactortest ${label} ... `);
	try {
		...
		const match = out.match(/reactortest PASS (\d+)\/(\d+)/);
...
const bin = buildBinary(file, "reactortest", true);
