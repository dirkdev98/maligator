...
addRuntimeBenchmark(
	"constructor-scalar-fields-computed-nonescaping",
	{
		owner: "constructor scalar replacement",
		category: "allocation-gc",
		mechanisms: ["constructor-call", "escape-analysis", "scalar-replacement"],
		inputShape: "nonescaping two-field base instance with computed values",
		unit: "constructed field sum",
		sourceSeam: "src/compiler/core/core-cross-call-transforms.ts",
		controls: ["constructor-scalar-fields-nonescaping"],
	},
	`const seed = Number(process.argv[2] ?? "1") & 255;
...
addRuntimeBenchmark(
	"string-cons-full-scan-32",
	{
		owner: "String.prototype.charCodeAt over a fresh two-child rope",
		category: "api-builtins",
		mechanisms: ["string-allocation", "string-character-access"],
		inputShape: "two selected flat 16-code-unit children, then a full scan",
		unit: "32-code-unit rope scan",
		sourceSeam: "runtime/src/heap_string.h",
		controls: ["string-concatenation"],
	},
	`const seed = Number(process.argv[2] ?? "1") & 255;
const left = Array.from({ length: 32 }, (_, index) =>
	\`left:\${String(seed + index).padStart(11, "0")}\`,
);
const right = Array.from({ length: 32 }, (_, index) =>
	\`right:\${String(seed + index).padStart(10, "0")}\`,
);
