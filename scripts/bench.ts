	...
	try {
		for (let index = 0; index < workloads.length; index++) {
			...
			try {
				...
				console.log(
					`    load outcomes       ${perfPerRequest(ic, "load_slow_mono_hits", requests).toFixed(1)} slow mono, ${perfPerRequest(ic, "load_own_table_hits", requests).toFixed(1)} own table, ${perfPerRequest(ic, "load_own_table_fills", requests).toFixed(1)} own table fills, ${perfPerRequest(ic, "load_shape_fills", requests).toFixed(1)} shape fills, ${perfPerRequest(ic, "inherited_fills", requests).toFixed(1)} inherited fills, ${perfPerRequest(ic, "load_missing_fills", requests).toFixed(1)} missing fills, ${perfPerRequest(ic, "load_plain_generic", requests).toFixed(1)} plain generic`,
				);
				console.log(
