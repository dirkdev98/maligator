...
| `AbortController`, `AbortSignal`                     |   P    | `abort`, `timeout`, reason, and `throwIfAborted` are in `runtime/src/runtime/events.c:304-483`; native coverage is `tests/native/web.test.ts:24`.                                                                                      | `AbortSignal.any`, range checks, dependent-signal lifetime, and `AbortError`/`TimeoutError` as `DOMException`; integrate cancellation into streams and fetch.                                                                                                             |
...
| `DOMException`                                       |   M    | Abort currently constructs plain `Error` objects (`runtime/src/runtime/events.c:323-330`); base64 and clone also throw ordinary JS errors.                                                                                             | Constructor, legacy code/name table, stack behavior, and use by abort, base64, clone, streams, and fetch.                                                                                                                                                                 |
| `Headers`                                            |   P    | Storage and methods are in `runtime/src/runtime/headers.c`; `tests/local/headers_iter.js` explicitly pins insertion-order, stored-case, uncombined v1 behavior.                                                                        | Web IDL coercion, sequence/record initialization, name/value validation and normalization, sorted lowercase live iteration, combination rules, guards, and `getSetCookie()`.                                                                                              |
...
| Web Streams interfaces                               |   M    | No `ReadableStream`, `WritableStream`, `TransformStream`, controller, reader/writer, BYOB, or queuing-strategy implementation was found.                                                                                               | Implement the Streams Standard as the common backpressure/cancellation primitive before streamed fetch, encoding, compression, and messaging transfer.                                                                                                                    |
