...
| `URLSearchParams`                                    |   P    | Parsing, mutation, sort, serialization, optional-value `delete`/`has` including explicit-`undefined` omission, and sequence/record initialization are in `runtime/src/runtime/url.c`. Every URL eagerly owns a stable `searchParams` object; query assignment and params mutation synchronize in both directions with traced GC ownership. The curated URLSearchParams slice has no expected failures. | Replace snapshot iteration with a dedicated branded live iterator and close remaining Web IDL descriptors/coercions against the URL WPT corpus.                                                                                                            |
