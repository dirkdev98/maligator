...
| `Event`, `EventTarget`                               |   P    | Constructors, dispatch, cancellation, and `once` are in `runtime/src/runtime/events.c`. Listener snapshot slots remain stable through nested dispatch, `once` removal precedes invocation, and removed/re-added listeners are handled correctly; native and pinned WPT stress coverage exercise these paths.                                                                                                                                                                                                                                  | Web IDL branding/descriptors, full event state, listener objects/options (`capture`, `passive`, `signal`), remaining mutation/reentrancy cases, and exception reporting.                                                                                   |
...
| Web Streams interfaces                               |   P    | `runtime/src/runtime/readable_stream.c` implements default `ReadableStream`, controller, and reader objects with queued and pending reads, start/pull/cancel algorithms, locking, disturbance, close/error, count- and callback-sized backpressure, promises, GC-stress coverage, and byte-backed Fetch Body integration. Constructor, invalid-strategy, and floating-point queue-total WPT pass in compiled/interpreted normal and stress modes.                                                                                             | Add byte/BYOB streams, writable/transform streams, tee/pipe/async iteration, transfer, broader WPT coverage, and integration with encoding, compression, and messaging.                                                                                    |
...
- [x] Add a real server-main testharness adapter, pin a WPT revision, record
      ...
      lane now runs 274 subtests from 36 byte-pinned Abort, Events, Encoding, Headers,
      Streams, timers, URL, and URLSearchParams files. One browser `Response` `Set-Cookie`
      guard case is an exact expected server-profile divergence; there are no
      unexpected or stale normal/stress results.
...
- [ ] Implement standard Web Streams, including byte/BYOB streams, backpressure,
      ...
      The default readable-stream/controller/reader vertical slice and custom size
      algorithms are implemented with an initial green WPT slice; byte/BYOB,
      writable/transform, composition, transfer, and broader WPT remain.
