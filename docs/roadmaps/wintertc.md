...
| `TextEncoder`, `TextDecoder`                         |   P    | UTF-8 `encode`, `encodeInto`, and buffered `decode` are in `runtime/src/runtime/web_globals.c`; `fatal` rejects malformed/truncated UTF-8, `ignoreBOM` controls BOM preservation, and readonly option getters are branded. The shared codec reports decode errors while preserving its lenient wrapper for Fetch and Buffer callers.                                           | Additional decoder labels/encodings, streaming state, `DataView`, remaining Web IDL coercion/descriptor details, and the complete Encoding Standard corpus.                                                                                                |
