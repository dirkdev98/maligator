...
| `AbortController`, `AbortSignal`                     |   P    | `abort`, `timeout`, `any`, reason, `throwIfAborted`, dependent propagation, and `DOMException` default reasons are in `runtime/src/runtime/web_events.c`; pinned static-abort and timeout-order WPT pass normally and under GC stress, with broader native stress coverage in `tests/native/web.test.ts`.                                                                                                                                                                                                                                         | Complete Web IDL range/coercion behavior and weak dependent-signal lifetime details; integrate cancellation into streams and fetch, then expand the applicable abort WPT corpus.                                                                           |
...
| `Headers`                                            |   P    | `runtime/src/runtime/web_headers.c` validates and lowercases names, normalizes HTTP whitespace, supports record and sequence initialization, combines duplicates, preserves `Set-Cookie`, honors constructor/new-target semantics, and provides branded, sorted, live iterators over a shared `%IteratorPrototype%`-derived prototype. The expanded pinned slice passes all standalone Headers behavior; its browser response-guard case is an exact expected failure because `Mal.serve` currently needs outbound `Response` cookies to survive. | Separate browser/network response filtering from server-produced responses, add remaining guards and forbidden-header policy, close Proxy/Web IDL record behavior and residual descriptors, and expand applicable WPT.                                     |
...
| global `fetch()`                                     |   M    | Despite the comment at `runtime/host_main.c:40`, `mal_fetch_install` only installs `Request`, `Response`, `Headers`, and `Mal.serve` (`runtime/src/runtime/web_fetch.c:921-992`).                                                                                                                                                                                                                                                                                                                                                                 | Outbound HTTP(S), redirects, abort, streamed upload/download, decompression, URL/headers integration, error mapping, and a documented server-runtime origin policy.                                                                                        |
| `Blob`, `File`, `FormData`                           |   M    | No runtime definitions were found; fetch bodies only accept strings and `BufferSource` in `runtime/src/runtime/web_fetch.c:160-175`.                                                                                                                                                                                                                                                                                                                                                                                                              | Immutable blob storage/slicing/streaming, file metadata, ordered multipart fields, multipart and urlencoded parsing/serialization, and Body integration.                                                                                                   |
| Web Streams interfaces                               |   P    | `runtime/src/runtime/web_readable_stream.c` implements default `ReadableStream`, controller, and reader objects with queued and pending reads, start/pull/cancel algorithms, locking, disturbance, close/error, count- and callback-sized backpressure, promises, GC-stress coverage, and byte-backed Fetch Body integration. Branded `CountQueuingStrategy` and `ByteLengthQueuingStrategy` globals provide shared size algorithms. The initial Streams WPT slice passes in compiled/interpreted normal and stress modes.                        | Add byte/BYOB streams, writable/transform streams, tee/pipe/async iteration, transfer, broader WPT coverage, and integration with encoding, compression, and messaging.                                                                                    |
...
runtime handler to the native server in `runtime/src/runtime/web_fetch.c:679-915` and
...
- [x] Add a real server-main testharness adapter, pin a WPT revision, record
      ...
      lane byte-pins applicable Abort, Events, Encoding, Headers, Streams, timers,
      URL, and URLSearchParams files. One browser `Response` `Set-Cookie` guard case
      is an exact expected server-profile divergence; generated output owns the raw
      execution counts and there are no unexpected or stale normal/stress results.
...
- [ ] Implement standard Web Streams, including byte/BYOB streams, backpressure,
      ...
      The default readable-stream/controller/reader vertical slice, custom size
      algorithms, and count/byte-length queuing strategies are implemented with an
      initial green WPT slice; byte/BYOB, writable/transform, composition, transfer,
      and broader WPT remain.
...
- [ ] Close Headers and URL/URLSearchParams semantics, then implement URLPattern.
      URLSearchParams now has stable URL association, optional-value semantics, and
      branded live iterators; static `URL.parse` now shares the Ada parser and returns
      a fresh associated URL on success. Broader URL/Web IDL coverage remains.
- [ ] Complete TextDecoder state/labels and add encoding streams. Stateful UTF-8,
      UTF-16LE, and UTF-16BE decoding now preserves partial sequences and BOM state
      across chunks and resets on final flush; the remaining Encoding Standard labels
      and encoding stream interfaces remain.
