...
| `TextEncoder`, `TextDecoder`                         |   P    | UTF-8 `encode`, `encodeInto`, and buffered `decode` are in `runtime/src/runtime/web_globals.c`; `fatal` rejects malformed/truncated UTF-8, `ignoreBOM` controls BOM preservation, readonly option getters are branded, and ArrayBuffer, TypedArray, and DataView byte windows are accepted with detached/resizable handling. The shared codec still has a lenient wrapper.     | Additional decoder labels/encodings, streaming state, remaining Web IDL coercion/descriptor details, and the complete Encoding Standard corpus.                                                                                                            |
