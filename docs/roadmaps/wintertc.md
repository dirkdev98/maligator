...
| `TextEncoder`, `TextDecoder`                         |   P    | UTF-8 `encode`, `encodeInto`, and buffered `decode` are in `runtime/src/runtime/web_globals.c`; the shared codec now replaces lone surrogates and malformed/overlong/out-of-range UTF-8 sequences, with Fetch and Buffer regression coverage.                                                                                                                                  | Decoder labels/encodings, `fatal`, `ignoreBOM`, streaming state, `DataView`, Web IDL coercion/branding, and the complete Encoding Standard corpus. Options remain ignored.                                                                                 |
