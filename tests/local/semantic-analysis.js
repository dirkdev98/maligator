...
//
// and some random try-outs.
//
// All useful to quickly verify semantic analysis
...
assert.compareArray = function (actual, expected, message) {
	...
	assert(
		false,
		`Actual ${format(actual)} and expected ${format(expected)} should have the same contents. ${message}`,
	);
};
...
var __propertyIsEnumerable = Function.prototype.call.bind(
...
function isWritable(obj, name, verifyProp, value) {
	var unlikelyValue =
		__isArray(obj) && name === "length" ? nonIndexNumericPropertyName : "unlikelyValue";
...
function verifyEqualTo(obj, name, value) {
	if (!isSameValue(obj[name], value)) {
		throw new Test262Error(
			"Expected obj[" + String(name) + "] to equal " + value + ", actually " + obj[name],
		);
	}
}
...
function verifyWritable(obj, name, verifyProp, value) {
	if (!verifyProp) {
		assert(
			__getOwnPropertyDescriptor(obj, name).writable,
...
function verifyEnumerable(obj, name) {
	assert(
		__getOwnPropertyDescriptor(obj, name).enumerable,
...
function verifyConfigurable(obj, name) {
	assert(
		__getOwnPropertyDescriptor(obj, name).configurable,
...
class TestClass {
	...
	static {
		this.staticField = 100;
		var hoistedVar = "in static block";
		let blockScoped = "also in static block";
	}

	methodWithVar() {
		if (true) {
			var functionScoped = "var hoists to method";
			let blockScoped = "let stays in block";
		}
		console.log(functionScoped); // works
		// console.log(blockScoped); // would fail
	}
}

// Var hoisting from nested blocks
function varInNestedBlocks() {
	if (true) {
		var hoistedFromIf = 1;
	}
	while (false) {
		var hoistedFromWhile = 2;
	}
	switch (1) {
		case 1:
			var hoistedFromSwitch = 3;
			break;
	}
	try {
		var hoistedFromTry = 4;
	} catch (e) {
		var hoistedFromCatch = 5;
	} finally {
		var hoistedFromFinally = 6;
	}
	for (var i = 0; i < 1; i++) {
		var hoistedFromFor = 7;
	}
	console.log(hoistedFromIf, hoistedFromWhile, hoistedFromSwitch);
}

// Var shadowing parameter - should NOT create new binding
function varShadowsParam(param1, param2) {
	var param1;
	console.log(param1);
}

// Multiple var declarations of same name - should merge
function multipleVarSameName() {
	var x = 1;
	if (true) {
		var x = 2;
	}
...
// Function declaration hoisting - function wins over var
function funcVsVar() {
	console.log(innerFunc);
	var innerFunc = "var wins?";
	function innerFunc() {
		return "function wins";
	}
}

// Function in block (sloppy mode)
...
// Var before function of same name - function wins
function varBeforeFunc() {
	var sameName = "var value";
	function sameName() {
		return "func value";
	}
...
// Destructuring var hoisting
function varDestructuring() {
	var { a, b: renamed } = { a: 1, b: 2 };
	var [first, second] = [1, 2];
	console.log(a, renamed, first, second);
}

// Function declaration in block - sloppy mode hoisting to function scope
function sloppyBlockFunc() {
...
// Var in labeled statement
function varInLabeled() {
...
// Complex parameter + var interaction
...
// Function hoisted before reference
function usageBeforeDecl() {
	return helper();
	function helper() {
		return "works";
	}
...
// Var with initializer in catch parameter shadow
function catchParamShadow() {
	try {
