...
const buckets = {};
const add = (k, p) => { (buckets[k] ??= []).push(p); };
for (const p of arr) {
  let src = "";
  try { src = fs.readFileSync(base + p, "utf8"); } catch (e) {}
  const fm = (src.match(/\/\*---([\s\S]*?)---\*\//) || [])[1] || "";
  const feat = (fm.match(/features:\s*\[([^\]]*)\]/) || [])[1] || "";
  const features = feat.split(",").map(s => s.trim()).filter(Boolean);
  const hasDefer = features.includes("import-defer") || /import\.defer/.test(src);
  const hasSource = features.includes("source-phase-imports") || /import\.source/.test(src);
  const hasAttrs = features.includes("import-attributes") || /\bwith\s*\{/.test(src) || /assert\s*:/.test(src);
  let k;
  ...
  else if (p.includes("/assignment-expression/")) k = "assignment-expr(computed-specifier)";
...
for (const p of (buckets["catch(error-path)"] || []).slice(0, 20)) console.log("  ", p.split("/").pop());
console.log("\n--- namespace samples ---");
for (const p of (buckets["namespace"] || []).slice(0, 20)) console.log("  ", p.split("/").pop());
console.log("\n--- root/other samples ---");
