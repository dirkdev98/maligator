...
const outDir = mkdtempSync(path.join(os.tmpdir(), "mal-text-decoder-streaming-"));
...
describe("TextDecoder streaming", () => {
	...
	beforeAll(() => {
		compiled = buildNativeBinary({
			fixture: "tests/local/text_decoder_streaming.js",
			name: "text-decoder-streaming-compiled",
			mainFile: HOST_MAIN,
			outDir,
		});
		interpreted = buildNativeBinary({
			fixture: "tests/local/text_decoder_streaming.js",
			name: "text-decoder-streaming-interpreted",
			compiled: false,
			mainFile: HOST_MAIN,
			outDir,
		});
