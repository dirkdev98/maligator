...
const outDir = mkdtempSync(path.join(os.tmpdir(), "mal-webidl-sequence-init-"));
const HOST_GC = { MAL_HOST_GC: "1" };

describe("WinterTC sequence initialization", () => {
	let compiled: string;
	let interpreted: string;

	beforeAll(() => {
		compiled = buildNativeBinary({
			fixture: "tests/local/webidl_sequence_init.js",
			name: "webidl-sequence-init-compiled",
			mainFile: HOST_MAIN,
			outDir,
		});
		interpreted = buildNativeBinary({
			fixture: "tests/local/webidl_sequence_init.js",
			name: "webidl-sequence-init-interpreted",
			compiled: false,
			mainFile: HOST_MAIN,
			outDir,
		});
