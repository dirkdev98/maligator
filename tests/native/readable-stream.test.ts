...
const outDir = mkdtempSync(path.join(os.tmpdir(), "mal-readable-stream-"));

describe("ReadableStream default mode", () => {
	let compiled: string;
	let interpreted: string;

	beforeAll(() => {
		compiled = buildNativeBinary({
			fixture: "tests/local/readable_stream.js",
			name: "readable-stream-compiled",
			mainFile: HOST_MAIN,
			outDir,
		});
		interpreted = buildNativeBinary({
			fixture: "tests/local/readable_stream.js",
			name: "readable-stream-interpreted",
			compiled: false,
			mainFile: HOST_MAIN,
			outDir,
		});
