...
it.each([
	...
	"preserves known-operation semantics with %s primordials and profiling %s",
	(primordials, profileEnabled) => {
		const fixture = "tests/local/known-operations.mjs";
		const expected = execFileSync(process.execPath, [fixture], { encoding: "utf8" });
		const outDir = mkdtempSync(join(tmpdir(), "mal-known-operations-"));
		try {
			const pair = buildBackendPairFromOneProgramImage({
				fixture,
				name: "known-operations",
				profileEnabled,
