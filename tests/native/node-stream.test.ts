...
const outDir = mkdtempSync(path.join(os.tmpdir(), "mal-node-stream-"));

describe("node:stream in-memory foundation", () => {
	...
	beforeAll(() => {
		esmCompiled = buildNativeBinary({
			fixture: "tests/local/node-stream.mjs",
			name: "node-stream-esm-compiled",
			mainFile: HOST_MAIN,
			outDir,
			nodeEnabled: true,
		});
		esmInterpreted = buildNativeBinary({
			fixture: "tests/local/node-stream.mjs",
			name: "node-stream-esm-interpreted",
			mainFile: HOST_MAIN,
			outDir,
			nodeEnabled: true,
			compiled: false,
		...
		cjsCompiled = buildNativeBinary({
			fixture: "tests/local/node-stream.cjs",
			name: "node-stream-cjs-compiled",
			mainFile: HOST_MAIN,
			outDir,
			nodeEnabled: true,
		});
		cjsInterpreted = buildNativeBinary({
			fixture: "tests/local/node-stream.cjs",
			name: "node-stream-cjs-interpreted",
			mainFile: HOST_MAIN,
			outDir,
			nodeEnabled: true,
			compiled: false,
	...
	it("passes under MAL_GC_STRESS + MAL_GC_VERIFY", () => {
		...
		assertResultPass(runToStdout(esmInterpreted, { env: STRESS_ENV }));
		assertResultPass(runToStdout(cjsCompiled, { env: STRESS_ENV }));
		assertResultPass(runToStdout(cjsInterpreted, { env: STRESS_ENV }));
	});
});
