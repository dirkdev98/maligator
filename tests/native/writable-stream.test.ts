...
const outDir = mkdtempSync(path.join(os.tmpdir(), "mal-writable-stream-"));

describe("WritableStream default mode", () => {
	let compiled: string;
	let interpreted: string;

	beforeAll(() => {
		compiled = buildNativeBinary({
			fixture: "tests/local/writable_stream.js",
			name: "writable-stream-compiled",
			mainFile: HOST_MAIN,
			outDir,
		});
		interpreted = buildNativeBinary({
			fixture: "tests/local/writable_stream.js",
			name: "writable-stream-interpreted",
			compiled: false,
			mainFile: HOST_MAIN,
			outDir,
		});
