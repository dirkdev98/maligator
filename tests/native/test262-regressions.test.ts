...
const corpusReady = existsSync(TEST262_METADATA.cacheFile) && existsSync(TEST262_METADATA.path);

describe.skipIf(!corpusReady)("test262 regression manifest", () => {
	it(
		"curated common-case tests still pass (no regression vs baseline)",
		() => {
			try {
				execFileSync(
					"node",
				...
				throw new Error(`test262 regression manifest failed:\n${e.stdout ?? ""}\n${e.stderr ?? ""}`);
			}
		},
		600_000,
	);
