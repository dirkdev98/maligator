...
type NativeReachingDefinitionAt = (
	register: number,
	ip: number,
) => ReadonlySet<number> | undefined;

/** Candidate-local reaching definitions: most functions and most registers do
 * not participate in a closed aggregate, so do not solve them eagerly. */
function lazyNativeReachingDefinitions(
	fn: VmFunction,
	cfg: NativeProofCfg,
): NativeReachingDefinitionAt {
	const cache = new Map<number, Array<ReadonlySet<number>>>();
	return (register, ip) => {
		let definitions = cache.get(register);
		if (definitions === undefined) {
			definitions = privateAggregateReachingDefinitionsForRegister(fn, cfg, register);
			cache.set(register, definitions);
		}
		return definitions[ip];
	};
}

function closedRecordMoveAliases(
	fn: VmFunction,
	seeds: ReadonlyArray<{ ip: number; register: number }>,
	reachingAt: NativeReachingDefinitionAt,
): { definitions: Set<number>; registers: Set<number> } {
	const definitions = new Set(seeds.map((seed) => seed.ip));
	const registers = new Set(seeds.map((seed) => seed.register));
	const movesBySource = new Map<
		number,
		Array<{
			ip: number;
			instruction: Extract<VmInstruction, { opcode: "MOVE" }>;
		}>
	>();
	for (let ip = 0; ip < fn.instructions.length; ip++) {
		const instruction = fn.instructions[ip]!;
		if (instruction.opcode !== "MOVE") continue;
		const moves = movesBySource.get(instruction.src) ?? [];
		moves.push({ ip, instruction });
		movesBySource.set(instruction.src, moves);
	}
	let changed = true;
	while (changed) {
		changed = false;
		for (const register of registers) {
			for (const move of movesBySource.get(register) ?? []) {
				if (definitions.has(move.ip)) continue;
				const reaching = reachingAt(register, move.ip);
				if (
					reaching !== undefined &&
					reaching.size > 0 &&
					[...reaching].every((definitionIp) => definitions.has(definitionIp))
				) {
					definitions.add(move.ip);
					registers.add(move.instruction.dst);
					changed = true;
				}
			}
		}
...
interface ClosedRecordCanonicalCounter {
	register: number;
	bound: number;
	initializerIp: number;
	incrementIp: number;
...
/** Exact `[0, bound)` counter proof shared by the closed record producer and
 * consumer loops. The deliberately small form keeps the aggregate certificate
 * independent from general range analysis: zero outside the natural loop, one
 * in-loop increment, a constant upper bound, and the canonical two-way header. */
function closedRecordCanonicalCounter(
	fn: VmFunction,
	loop: NativeProofCfg["loops"][number],
	reachingAt: NativeReachingDefinitionAt,
	maximumBound: number,
): ClosedRecordCanonicalCounter | undefined {
	const constantAt = (register: number, useIp: number): number | undefined => {
		const definitions = reachingAt(register, useIp);
		if (definitions?.size !== 1) return undefined;
		const definition = fn.instructions[[...definitions][0]!];
		return definition?.opcode === "CREATE_NUMBER" || definition?.opcode === "CREATE_F64"
			? definition.value
			: undefined;
	};
	for (const ip of loop.body) {
		const comparison = fn.instructions[ip];
		const branch = fn.instructions[ip + 1];
		const exit = fn.instructions[ip + 2];
		if (
			comparison?.opcode !== "BINARY" ||
			comparison.operator !== "<" ||
			branch?.opcode !== "JUMP_IF" ||
			...
			!loop.body.has(branch.targetIp) ||
			exit?.opcode !== "JUMP" ||
			loop.body.has(exit.targetIp)
		) {
			continue;
		}
		const bound = constantAt(comparison.right, ip);
		if (!Number.isSafeInteger(bound) || bound! <= 0 || bound! > maximumBound) continue;
		const counter = comparison.left;
		const definitions = reachingAt(counter, ip);
		if (definitions?.size !== 2) continue;
		let initializerIp = -1;
		let incrementIp = -1;
		for (const definitionIp of definitions) {
			const definition = fn.instructions[definitionIp];
			if (
				(definition?.opcode === "CREATE_NUMBER" || definition?.opcode === "CREATE_F64") &&
				definition.value === 0 &&
				!loop.body.has(definitionIp)
			) {
				initializerIp = definitionIp;
		...
		if (initializerIp < 0 || incrementIp < 0) continue;
		return {
			register: counter,
			bound: bound!,
			initializerIp,
...
/** Reject irreducible/multi-entry loops, nested cycles, and early exits. The
 * counter shape alone proves the values seen at the header; this control proof
 * is what proves the producer actually executes one append for every ordinal. */
function closedRecordHasExactCanonicalControl(
	fn: VmFunction,
	cfg: NativeProofCfg,
	loop: NativeProofCfg["loops"][number],
	counter: ClosedRecordCanonicalCounter,
): boolean {
	const containedLoops = cfg.loops.filter(
		(candidate) => loop.body.has(candidate.header) && loop.body.has(candidate.backedge),
	);
	if (
		containedLoops.length !== 1 ||
		containedLoops[0]!.header !== loop.header ||
		containedLoops[0]!.backedge !== loop.backedge
	) {
		return false;
	}
	for (const ip of loop.body) {
		for (const predecessor of cfg.predecessors[ip]!) {
			if (!loop.body.has(predecessor) && ip !== loop.header) return false;
		}
		for (const successor of cfg.successors[ip]!) {
			if (
				!loop.body.has(successor) &&
				!(ip === counter.comparisonIp + 1 && successor === counter.comparisonIp + 2)
			) {
				return false;
			}
		}
	}
	const exit = fn.instructions[counter.comparisonIp + 2];
	return (
		cfg.predecessors[loop.header]!.filter((ip) => loop.body.has(ip)).length === 1 &&
		cfg.predecessors[loop.header]!.includes(loop.backedge) &&
		cfg.dominators[loop.backedge]!.has(counter.incrementIp) &&
		exit?.opcode === "JUMP" &&
		exit.targetIp === counter.exitIp
	);
}

function sameVmStringConstant(
	definition: VmDefinition,
	left: number,
	right: number,
): boolean {
	const leftUnits = definition.stringConstants[left];
	const rightUnits = definition.stringConstants[right];
	return (
		leftUnits !== undefined &&
		rightUnits !== undefined &&
		leftUnits.length === rightUnits.length &&
		leftUnits.every((unit, index) => unit === rightUnits[index])
	);
}

/**
 * Prove one closed record-Array region:
 *
 *   const rows = [];
 *   for (let i = 0; i < N; i++) rows.push({ ...same static shape... });
 *   for (...) for (let i = 0; i < N; i++) rows[i].field ...;
 *
 * The Array and every record remain function-private. All appends execute the
 * locked exact builtin, every consumer index is range-proven, and record uses
 * are limited to existing own static data fields. The physical objects remain
 * allocated, so identity and GC behavior are unchanged; native code only skips
 * redundant Array/property dispatch. The complete ordinary instructions remain
 * the interpreted/rejected-proof twin, and this post-wire proof is rebuilt from
 * semantic facts rather than serialized emitter authority.
 */
function annotateNativeClosedRecordArrayRegions(definition: VmDefinition): void {
	const lockedFact = vmSemanticProtectorGuard(
		definition.semanticProtectors,
		"primitive-methods",
	);
	for (const fn of definition.functions) {
		delete fn.nativeClosedRecordArrayRegions;
		for (const instruction of fn.instructions) {
		...
		if (
			lockedFact === undefined ||
			!vmGuardIsWorldInvariant(lockedFact) ||
			fn.isGenerator ||
			fn.isAsync ||
			fn.needsArguments ||
			fn.mappedArguments ||
		...
		const knownPushes = fn.instructions.filter(
			(instruction) =>
		...
		const shapedObjects = fn.instructions.filter(
			(instruction) => instruction.opcode === "CREATE_OBJECT_SHAPED",
		);
		if (
			// The first cost boundary admits one aggregate construction per
			// function; broader selection waits for a shared region cost model.
			allocations.length !== 1 ||
			knownPushes.length !== 1 ||
			shapedObjects.length !== 1 ||
			!fn.instructions.some((instruction) => instruction.opcode === "LOAD_PROPERTY") ||
			!fn.instructions.some(
		...
		const cfg = buildNativeProofCfg(fn);
		const reachingAt = lazyNativeReachingDefinitions(fn, cfg);

		// One region per function is an intentional first cost boundary. It keeps
		// overlapping aggregate aliases out until a real region forest/cost model
		// chooses between them.
		for (const allocation of allocations) {
			const arrayAliases = closedRecordMoveAliases(
				fn,
				[{ ip: allocation.ip, register: allocation.instruction.dst }],
				reachingAt,
			);
			const isArrayUse = (register: number, ip: number): boolean => {
				const definitions = reachingAt(register, ip);
				return (
					definitions !== undefined &&
					definitions.size > 0 &&
					[...definitions].every((definitionIp) =>
						arrayAliases.definitions.has(definitionIp),
					)
				);
			};
			...
			const pushes: Array<{
				ip: number;
				argument: number;
				guard: VmRegionLicense["guard"];
			}> = [];
			const elementLoads: Array<{
				ip: number;
				instruction: Extract<VmInstruction, { opcode: "LOAD_PROPERTY" }>;
			}> = [];
			let closed = true;
			for (let ip = 0; ip < fn.instructions.length && closed; ip++) {
				const instruction = fn.instructions[ip]!;
				const arrayUseRegisters = [...arrayAliases.registers].filter(
					(register) =>
						isArrayUse(register, ip) &&
						nativeInstructionUsesRegister(instruction, register),
				);
				if (arrayUseRegisters.length === 0) continue;
				if (
					instruction.opcode === "MOVE" &&
					arrayUseRegisters.length === 1 &&
					instruction.src === arrayUseRegisters[0]
				) {
					continue;
				}
				if (
				...
				if (
					...
					arrayUseRegisters.length === 1 &&
					instruction.object === arrayUseRegisters[0] &&
					!isArrayUse(instruction.key, ip)
				) {
					elementLoads.push({ ip, instruction });
					continue;
				}
				closed = false;
			}
			if (!closed || pushes.length !== 1 || elementLoads.length === 0) {
				continue;
			}

			const push = pushes[0]!;
			const producerLoop = cfg.loops
				.filter((loop) => loop.body.has(push.ip))
				.sort((left, right) => left.body.size - right.body.size)[0];
			const producerCounter =
				producerLoop === undefined
					? undefined
					: closedRecordCanonicalCounter(fn, producerLoop, reachingAt, 65_536);
			if (
				producerLoop === undefined ||
				producerCounter === undefined ||
				!closedRecordHasExactCanonicalControl(fn, cfg, producerLoop, producerCounter) ||
				producerLoop.body.has(allocation.ip) ||
				!cfg.dominators[producerLoop.header]!.has(allocation.ip) ||
				!cfg.dominators[producerLoop.backedge]!.has(push.ip) ||
				!cfg.dominators[push.ip]!.has(producerLoop.header)
			) {
				continue;
			}

			const traceMoveRoot = (register: number, useIp: number): number | undefined => {
				let currentRegister = register;
				let currentUseIp = useIp;
				const seen = new Set<number>();
				while (true) {
					const definitions = reachingAt(currentRegister, currentUseIp);
					if (definitions?.size !== 1) return undefined;
					const definitionIp = [...definitions][0]!;
					if (seen.has(definitionIp)) return undefined;
					seen.add(definitionIp);
					const instruction = fn.instructions[definitionIp];
					if (instruction?.opcode !== "MOVE") return definitionIp;
					currentRegister = instruction.src;
					currentUseIp = definitionIp;
				}
			};
			const producerObjectIp = traceMoveRoot(push.argument, push.ip);
			const producerObject =
				producerObjectIp === undefined ? undefined : fn.instructions[producerObjectIp];
			if (
				producerObject?.opcode !== "CREATE_OBJECT_SHAPED" ||
				!producerLoop.body.has(producerObjectIp!) ||
				!cfg.dominators[push.ip]!.has(producerObjectIp!) ||
				producerObject.count === 0 ||
				new Set(
					producerObject.keyStringIndices.map((stringIndex) =>
						definition.stringConstants[stringIndex]?.join(","),
					),
				).size !== producerObject.count
			) {
				continue;
			}
			...
			for (const load of elementLoads) {
				const consumerLoop = cfg.loops
					.filter(
						(loop) => loop.body.has(load.ip) && loop.backedge > producerLoop.backedge,
					)
					.sort((left, right) => left.body.size - right.body.size)[0];
				const counter =
					consumerLoop === undefined
						? undefined
						: closedRecordCanonicalCounter(
								fn,
								consumerLoop,
								reachingAt,
								producerCounter.bound,
							);
				if (
					consumerLoop === undefined ||
					counter === undefined ||
					counter.bound !== producerCounter.bound ||
					load.instruction.key !== counter.register ||
					!cfg.dominators[load.ip]!.has(producerCounter.exitIp) ||
					!cfg.dominators[load.ip]!.has(consumerLoop.header) ||
					!cfg.dominators[consumerLoop.backedge]!.has(load.ip)
				) {
			...
			const recordAliases = closedRecordMoveAliases(
				fn,
				[
					{ ip: producerObjectIp!, register: producerObject.dst },
					...elementLoads.map((load) => ({
						ip: load.ip,
						register: load.instruction.dst,
					})),
				],
				reachingAt,
			);
			const isRecordUse = (register: number, ip: number): boolean => {
				const definitions = reachingAt(register, ip);
				return (
					definitions !== undefined &&
					definitions.size > 0 &&
					[...definitions].every((definitionIp) =>
						recordAliases.definitions.has(definitionIp),
					)
				);
			};
			const slotOf = (stringIndex: number): number =>
				producerObject.keyStringIndices.findIndex((candidate) =>
					sameVmStringConstant(definition, candidate, stringIndex),
				);
			...
			for (let ip = 0; ip < fn.instructions.length && closed; ip++) {
				const instruction = fn.instructions[ip]!;
				const recordUseRegisters = [...recordAliases.registers].filter(
					(register) =>
						isRecordUse(register, ip) &&
						nativeInstructionUsesRegister(instruction, register),
				);
				if (recordUseRegisters.length === 0) continue;
				if (
					instruction.opcode === "MOVE" &&
					recordUseRegisters.length === 1 &&
					instruction.src === recordUseRegisters[0]
				) {
					continue;
				}
				if (instruction.opcode === "CALL" || instruction.opcode === "CALL_BUILTIN") {
					const argumentsUsed = instruction.arguments.filter((operand) => {
						const decoded = decodeVmValueOperand(operand);
						return decoded.kind === "register" && isRecordUse(decoded.register, ip);
					});
					const receiver = decodeVmValueOperand(instruction.thisValue);
					if (
						ip === push.ip &&
						argumentsUsed.length === 1 &&
						receiver.kind === "register" &&
						isArrayUse(receiver.register, ip)
					) {
						continue;
					}
				}
				if (
					instruction.opcode === "LOAD_PROPERTY_STATIC" &&
					recordUseRegisters.length === 1 &&
					instruction.object === recordUseRegisters[0]
				) {
					const slot = slotOf(instruction.stringIndex);
					if (slot >= 0) {
						accesses.push({ ip, kind: "load", slot });
						continue;
					}
				}
				if (
					...
					recordUseRegisters.length === 1 &&
					instruction.object === recordUseRegisters[0] &&
					!isRecordUse(instruction.value, ip)
				) {
					const slot = slotOf(instruction.stringIndex);
					if (slot >= 0) {
						accesses.push({ ip, kind: "store", slot });
						continue;
					}
				}
				closed = false;
			}
			if (!closed || accesses.length < 2) {
				continue;
			}
			const license = vmRegionLicense(
				[lockedFact, ...pushes.map((entry) => entry.guard)],
				"none",
			);
			if (license === undefined || !vmGuardIsWorldInvariant(license.guard)) {
				continue;
			}

			for (const load of elementLoads) {
				load.instruction.nativeClosedRecordArrayAccess = {
					allocationIp: allocation.ip,
				};
			}
			fn.nativeClosedRecordArrayRegions = [
				{
					license,
					allocationIp: allocation.ip,
					producerObjectIp: producerObjectIp!,
					length: producerCounter.bound,
					elementLoadIps: elementLoads.map((load) => load.ip),
					accesses,
				},
			];
			break;
...
function emitVmDefinitionSource(
	...
	if (useCompiled) {
		...
		annotateNativeClosedRecordArrayRegions(definition);
