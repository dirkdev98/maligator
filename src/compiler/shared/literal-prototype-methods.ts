...
const methods: Readonly<Record<LiteralReceiverKind, ReadonlyArray<string>>> = {
	object: [
		"__defineGetter__",
		"__defineSetter__",
		"__lookupGetter__",
...
export function generateLiteralPrototypeMethods(): string {
	const lines = literalPrototypeMethods.map(
		(method, index) =>
			`MAL_LITERAL_METHOD(${index}, ${method.prototype}, ${JSON.stringify(method.key)})`,
	);
	return `${lines.join("\n")}\n`;
}
