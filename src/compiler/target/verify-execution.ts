...
function verifyDirectEntries(
	program: ExecutionProgram,
	models: ReadonlyArray<FunctionModel>,
): void {
	for (const [functionIndex, model] of models.entries()) {
		const { fn } = model;
		...
		for (const [entryIndex, entry] of fn.directEntries.entries()) {
			...
			const variantModel: FunctionModel = {
				...model,
				fn: {
					...fn,
			...
			verifyInstructionOperands(variantModel);
			verifyParallelCopies(variantModel);
			verifyGcRoots(variantModel, core);
		}
	}

	for (const [functionIndex, model] of models.entries()) {
		for (const { instructions } of model.fn.blocks) {
					...
					const actual =
						register === undefined
							...
							: register < 0
								...
								: model.fn.registerRepresentations[register];
					if (actual !== expected) {
						fail(
...
export function verifyExecutionProgram(program: ExecutionProgram): void {
	...
	verifyDirectEntries(program, models);
}
