...
const keys = Array.from({ length: 4_096 }, (_, index) => `key:${seed}:${index}`);
const misses = Array.from({ length: 4_096 }, (_, index) => `miss:${seed}:${index}`);
...
runRuntimeGapCase("map-get-miss-4096-selected-string-key", run);
