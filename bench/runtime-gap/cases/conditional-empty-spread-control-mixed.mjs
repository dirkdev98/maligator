...
function run(scale) {
	...
	for (let index = 0; index < operations; index++) {
		...
		const previous = index & 1 ? (seed + index - 1) & 255 : undefined;
		const record = previous === undefined ? { value } : { value, previous };
...
runRuntimeGapCase("conditional-empty-spread-control-mixed", run, verify);
