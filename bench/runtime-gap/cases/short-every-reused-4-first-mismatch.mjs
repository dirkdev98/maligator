...
const expectedArrays = Array.from({ length: 32 }, (_, arrayIndex) =>
	Array.from({ length: 4 }, (_, index) => (seed + arrayIndex + index) & 255),
);
...
function run(scale) {
	...
	for (let index = 0; index < operations; index++) {
		...
		checksum += ((currentExpected = expected), values.every(reusedPredicate)) ? 1 : 0;
	}
	return { checksum: checksum >>> 0, operations };
}

runRuntimeGapCase("short-every-reused-4-first-mismatch", run);
