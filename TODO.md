...
- [ ] Complete representative PGO training and performance acceptance; tune the
      ...
      faster in total measured time, with mixed holdouts. D068 adds an explicit
      full frontend holdout with exact Node parity; three measured pairs total
      +0.03% under PGO with mixed signs. Build a size-matched static control before
      tuning more PGO scores: prove a runtime advantage at equal code size or
      less code at equal runtime on trained and held-out inputs, and account for
      training cost before changing the default. Do not tune against D045's old
