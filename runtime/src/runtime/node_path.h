 ...
 * `extname`, `isAbsolute`, `join`, `normalize`, `relative`, `resolve`, the POSIX
 * `delimiter` and `sep` constants, and a `default` object carrying the same
 * values. The `launch` context is unused here. Referenced by name from the emitted
 * install manifest (mal_host_install_node_path); the compiler's dead-code
 * elimination keeps the whole module TU out of programs that never import
 * `node:path`.
 */
