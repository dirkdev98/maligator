 ...
 */
static bool mal_builtin_array_try_get(MalValue this_value, u32 index, MalValue *out) {
...
static MalValue mal_builtin_array_get(MalValue this_value, u32 index) {
    MalValue element = mal_value_new_undefined();
    mal_builtin_array_try_get(this_value, index, &element);
    return element;
}
...
/**
 * Call a (element, index, array) style callback, propagating abnormal
 * completions to the VM.
 */
static bool mal_builtin_array_invoke(MalVm *vm, MalValue callback, MalValue element, u32 index, MalValue this_value, MalValue *out) {
    MalValue args[] = {element, mal_value_from_i32((i32) index), this_value};
    MalCompletion completion = mal_vm_call_value(vm, callback, mal_value_new_undefined(), args, 3);
...
static bool mal_builtin_array_callback_guard(MalValue this_value, const MalValue *args, i32 arg_count) {
    return mal_value_is_array_object(this_value) && arg_count >= 1 && mal_value_is_callable(args[0]);
}
...
static MalValue mal_builtin_array_from(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    for (u32 index = 0; index < length; index++) {
        MalValue element = mal_builtin_array_get(source, index);
...
static MalValue mal_builtin_array_map(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    if (!mal_builtin_array_callback_guard(this_value, args, arg_count)) {
        return mal_value_new_undefined();
    }
    ...
    for (u32 index = 0; index < length; index++) {
        MalValue element;
        if (!mal_builtin_array_try_get(this_value, index, &element)) {
            continue;
        }

        MalValue mapped;
        if (!mal_builtin_array_invoke(vm, args[0], element, index, this_value, &mapped)) {
            return mal_value_new_undefined();
        }
...
static MalValue mal_builtin_array_for_each(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    for (u32 index = 0; index < length; index++) {
        ...
        MalValue ignored;
        if (!mal_builtin_array_invoke(vm, args[0], element, index, this_value, &ignored)) {
            return mal_value_new_undefined();
        }
    }
...
static MalValue mal_builtin_array_filter(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    for (u32 index = 0; index < length; index++) {
        ...
        MalValue selected;
        if (!mal_builtin_array_invoke(vm, args[0], element, index, this_value, &selected)) {
            return mal_value_new_undefined();
        }
...
static MalValue mal_builtin_array_find(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    u32 length = mal_builtin_array_length(this_value);
    for (u32 index = 0; index < length; index++) {
        MalValue element = mal_builtin_array_get(this_value, index);

        MalValue matched;
        if (!mal_builtin_array_invoke(vm, args[0], element, index, this_value, &matched)) {
            return mal_value_new_undefined();
        }
...
static MalValue mal_builtin_array_includes(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    for (u32 index = start; index < length; index++) {
        ...
        if (mal_builtin_array_same_value_zero(mal_builtin_array_get(this_value, index), search)) {
            return mal_value_new_boolean(true);
        }
    }
...
static MalValue mal_builtin_array_pop(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    MalValue element = mal_builtin_array_get(this_value, length - 1);
...
static MalValue mal_builtin_array_shift(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    MalValue first = mal_builtin_array_get(this_value, 0);
...
static MalValue mal_builtin_array_unshift(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    for (u32 moved = length; moved > 0; moved--) {
        ...
        MalValue element;
        if (mal_builtin_array_try_get(this_value, from, &element)) {
...
static MalValue mal_builtin_array_slice(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    for (u32 index = 0; index < result_length; index++) {
        MalValue element;
        if (mal_builtin_array_try_get(this_value, start + index, &element)) {
            mal_object_set((MalObject *) result, mal_builtin_array_index_key(index), element);
        }
    }
    ...
    for (i32 i = -1; i < arg_count; i++) {
        ...
        if (mal_value_is_array_object(source)) {
            ...
            for (u32 index = 0; index < source_length; index++) {
                MalValue element;
                if (mal_builtin_array_try_get(source, index, &element)) {
...
static MalValue mal_builtin_array_at(MalVm *vm, MalValue this_value, const MalValue *args, i32 arg_count) {
    ...
    return mal_builtin_array_get(this_value, (u32) relative);
}
